// ==========================================================================
// Las cuatro secciones de un equipo: partidos, clasificación, horario y
// plantilla.
//
// Están aquí, como componentes sueltos, porque cada una se pinta en SU PROPIA
// pantalla (`app/equipo/[equipoId]/[seccion].tsx`). Antes convivían en una
// sola pantalla con un control segmentado de cuatro posiciones arriba, y ese
// control era el problema: «Clasificación» no cabe en un cuarto de pantalla y
// salía cortada, así que había que adivinar qué decía cada pestaña.
//
// Separarlas además deja sitio. La tabla de clasificación se lee mejor a
// pantalla completa, y la lista de partidos ya no compite por el alto con un
// selector que estaba siempre puesto aunque no se usara.
//
// De dónde sale cada cosa:
//   · Partidos y clasificación → federaciones, vía /api/competicion (que hoy
//     lee de Postgres). Solo se leen; el que manda es el acta de la federación.
...
export function Partidos({
  ...
  // El equipo llega por prop y no del «equipo activo» de la sesión: cada
  // sección vive en una ruta con su id dentro, así que ya no hay un estado
  // global que pueda ir por detrás de lo que se está mirando.
  equipo: Equipo
...
export function Horario({
  ...
  return (
    <View style={{ gap: espacio.md }}>
      {mando ? (
        <Boton icono="create-outline" tono="secundario" onPress={() => router.push(`/entrenamientos?equipo=${equipo.id}`)}>
...
export function Plantilla({ gente, equipo }: { gente: Usuario[]; equipo: Equipo }) {
