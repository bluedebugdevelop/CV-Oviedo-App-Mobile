...
import { escucharTodosLosEquipos } from '../../lib/firebase/equipos'
import {
  POSICIONES,
  ROLES,
  etiquetaRol,
...
function TarjetaUsuario({ usuario, equipos }: { usuario: Usuario; equipos: Equipo[] }) {
  ...
  return (
    <Tarjeta style={[e.usuario, !usuario.activo ? { opacity: 0.6 } : null]}>
      <View style={{ flex: 1, gap: 3 }}>
        ...
        <View style={e.etiquetas}>
          <Etiqueta fondo={paleta.fondo} texto={paleta.texto}>
            {etiquetaRol(usuario.rol).toUpperCase()}
          </Etiqueta>
          {suyos.length > 0 ? (
...
function FormularioAlta({
  ...
  const listo = nombre.trim().length >= 3 && emailOk && clave.length >= 6
      ...
      await crearUsuario(
        {
          ...
          dorsal: rol === 'jugador' ? dorsal.trim() : '',
          posicion: rol === 'jugador' ? (posicion ?? '') : '',
        },
        perfil!.uid,
      )
  ...
  return (
    <Tarjeta style={{ marginBottom: espacio.lg }}>
      ...
      />
      <Secundario>{ROLES.find((r) => r.valor === rol)?.explicacion}</Secundario>
...
const e = StyleSheet.create({
  ...
  usuario: { flexDirection: 'row', alignItems: 'center', gap: espacio.md, padding: espacio.md },
